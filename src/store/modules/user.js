...
import { getToken, getGames, setGames, removeGames, setGameMark, removeGameMark, getGameMark, setToken, removeToken, setName, getName, removeName, removeSlider, getSlider, setSlider, removePermission } from '@/utils/auth'
...
const actions = {
  ...
  getGames({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      getUserGames({ userName: username, token: token }).then(response => {
        var data = response.data
        if (!data) {
          reject('Verification failed, please Login again.')
        }
